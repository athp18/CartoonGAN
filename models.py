...
class Generator(nn.Module):
    ...
    def __init__(self, input_channels=3, output_channels=3):
        ...
        self.encoder = nn.Sequential(*list(resnet.children())[:-3])  # Include up to Layer3
...
class CycleGAN:
    ...
    def train_step(self, real_A, real_B):
        ...
        with torch.amp.autocast(device_type='cuda'):
            ...
            loss_D_real = self.criterion_GAN(pred_real, torch.ones_like(pred_real).to(self.device))
            
            fake_A = self.fake_A_buffer.push_and_pop(fake_A)
            pred_fake = self.D_A(fake_A.detach())
            loss_D_fake = self.criterion_GAN(pred_fake, torch.zeros_like(pred_fake).to(self.device))
