                        ...
                        if i % config['sample_frequency'] == 0:
                            with torch.no_grad():
                                ...
                                save_comparison_grid(
                                    ...
                                    os.path.join(samples_dir, f'samples_epoch_{epoch+1}_batch_{i}.png')
                                )

                        # Periodic checkpoint saving
                        if i > 0 and i % 1000 == 0:
                            checkpoint_name = f'periodic_checkpoint_epoch_{epoch+1}_batch_{i}.pth'
                            model.save_checkpoint(epoch + 1, epoch_metrics, checkpoint_name)
                            
                    except Exception as e:
                        print(f"\nError during batch processing: {e}")
                        # Save emergency checkpoint
                        emergency_name = f'emergency_checkpoint_epoch_{epoch+1}_batch_{i}.pth'
...
if __name__ == 'main':
    ...
    try:
        ...
        print(f"{'Starting fresh training' if resume_from is None else f'Resuming from {resume_from}'}")
